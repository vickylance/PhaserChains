...
function Main() {
  ...
  this.do_layout = function(layout) {
    ...
    var tip =
      '<div style="padding:2em;" class="member-help">select a method chain to see the help.</div>';
    ...
    switch (layout) {
      case 0:
        ...
        html +=
          '       <span id="method-chains-title" class="section-title">Method Chains</span>';
        html += '       <div id="chain-list" class="chain-list"/>';
        html += "   </td>";
        html += '   <td style="width:50%">';
        html +=
          '       <span id="examples-title" class="section-title">Examples</span>';
        ...
        html +=
          '       <div id="help" class="help-horizontal">' + tip + "</div>";
        html += "   </td>";
        html += "</tr>";
        break;
      case 1:
  ...
  this.update_bookmarks = function() {
    ...
    for (i = 0; i < this.bookmarks.length; i += 1) {
      html +=
        '<span raw-link=true class="raw-link bookmark link">' +
        this.bookmarks[i] +
        '</span><span bookmark-del="' +
        i +
        '" class="del-img"></span>';
    }
    $("#bookmarks").html(html);
  ...
  this.build_chains = function(
    ...
    var prop_list = unit.properties["public"].concat(
      unit.properties["protected"]
    );
  ...
  this.update_examples_viewer = function() {
    ...
    if (query.length >= 3) {
      ...
      for (i = 0; i < filelist.length; i += 1) {
        ...
        if (this.matches(query_split, file, result)) {
          display_line =
            result[0] +
            '<span class="highlight-example">' +
            result[1] +
            "</span>" +
            result[2];
          html += '<div example-line="' + i + '" class="example-list-item">';
          html +=
            '<a target="_blank" href="https://github.com/photonstorm/phaser-examples/blob/master/';
          if (file.indexOf("labs") === 0) {
            html += file + '" class="example-link" style="float:left">';
          } else {
            html +=
              "examples/" + file + '" class="example-link" style="float:left">';
          }
          html += display_line + "</a>";
          html += "</div>";
          count += 1;
    ...
    $("#examples-title").html(
      count > 200 ? "Examples (200+)" : "Examples (" + count + ")"
    );
    if (html.length === 0) {
      html =
        '<span style="padding:2em;display:inline-block;">examples not found...</span>';
    }
    ...
    if (query.length >= 3) {
      ...
      for (i = 0; i < chains.length; i += 1) {
        ...
        if (this.matches(query_split, chain, result)) {
          ...
          var posfix = result[2];
          var img =
            item.is_property || item.is_const ? "icon-field" : "icon-method";
          img = '<span class="' + img + '"></span>';
          chain =
            prefix + '<span class="highlight">' + matching + "</span>" + posfix;
          html +=
            '<div chain-id="' +
            i +
            '" + class="chain-list-item depth-' +
            item.depth +
            '">' +
            img +
            chain +
      ...
      $("#method-chains-title").html(
        "Method Chains (" + (truncated ? max + "+" : max - limit) + ")"
      );
    }
    if (html === "") {
      html =
        '<span style="padding:2em;display:inline-block;">chains not found...</span>';
    }
  ...
  this.show_const_help = function(chain_item) {
    ...
    if (cls_info["class"].help) {
      ...
      html +=
        '<span id="class-help" class="member-help">' + help + "</span>\n\n";
    }

    html +=
      '<span style="color:purple">class</span> <span class="link">' +
      cls_info["class"].name +
      "</span>";
    if (cls_info["class"]["extends"]) {
      html +=
        ' <span style="color:purple">extends</span> <span class="link">' +
        cls_info["class"]["extends"] +
        "</span>";
    }
    html += " {\n";
    if (const_obj.help) {
      html +=
        '\n    <span class="member-help">' +
        repl(const_obj.help, /\\n/g, "\n    ") +
        "</span>\n\n";
    } else if (const_obj.inlineHelp) {
      html +=
        '\n    <span class="member-help">' +
        repl(const_obj.inlineHelp, /\\n/g, "\n    ") +
        "</span>\n\n";
    }
    ...
    html += "}\n";
    html +=
      '\n<a href="http://docs.phaser.io/' +
      cls +
      ".html#" +
      member +
      '" target="_black">(docs.phaser.io)</a>';
    html += "</pre>";

    $("#help").html(html);

    $("#show-more-help").click(
      (function(help) {
        return function() {
  ...
  this.show_method_help = function(chain_item) {
    ...
    if (method.returns) {
      html +=
        '\n\n        returns <span class="member-help">' +
        repl(method.returns.help, /\\n/g, "\n    ") +
        "</span>";
    }
